...
5,000 benign prompts were processed through Llama-3.1-8B-Instruct. SAE features were extracted from the last-token hidden state at layer 19 using a Goodfire pre-trained SAE (65,536 features). The top 512 most variable features were selected for Mahalanobis distance computation, using Ledoit-Wolf shrinkage to regularize the covariance estimate.
...
| Self-validation (first 500) | 90% validated, 7.2% probably safe, 2.8% possibly safe, 0% dangerous |
...
SAE features were extracted from the attack prompts (not responses) using the same Llama-3.1-8B-Instruct + SAE pipeline. Mahalanobis distances were computed against the Phase 1 baseline.

| Class | n | Mean Distance | Std | % Outside Envelope |
|---|---|---|---|---|
| Benign | 200 | 49.6 | 14.6 | 100% |
...
The SOTIF envelope was evaluated as a binary classifier: **can it detect adversarial prompts?** The evaluation dataset combined 500 benign prompts (Phase 1) with 448 adversarial episodes (Phase 2). A prompt was classified as adversarial if its Mahalanobis distance exceeded the Q95 baseline threshold (20.9).
...
The envelope reliably detects that an adversarial attack is occurring. It does **not** distinguish successful jailbreaks from refused attacks (best F1 = 0.43 for that task) --- both produce similar activation geometries. The envelope answers "is someone trying to attack?" not "did the attack succeed?"

### Feature Importance --- Which SAE Features Drive Detection?

Feature selection via mutual information, univariate AUC, and L1-penalized logistic regression identified the 10 most predictive SAE features (of 65,536). Auto-interpretability labels were generated by running 3,000 C4 text samples through the SAE on an A40 GPU and prompting the model to describe each feature's activation pattern.

**Features that activate on adversarial prompts (higher in attacks):**

| SAE Index | Label | Adversarial Mean | Benign Mean | Ratio |
|---|---|---|---|---|
| 25763 | Adversarial scenario framing | 0.308 | 0.048 | 6.4x |
| 23820 | Technical enumeration requests | 0.291 | 0.085 | 3.4x |
| 41237 | Academic pretext patterns | 0.249 | 0.055 | 4.5x |

**Features that activate on benign prompts (suppressed in attacks):**

| SAE Index | Label | Benign Mean | Adversarial Mean | Ratio |
|---|---|---|---|---|
| 62703 | Routine knowledge queries | 0.331 | 0.013 | 25x |
| 48927 | Direct benign requests | 0.108 | 0.001 | 108x |
| 33189 | Standard task completion | 0.168 | 0.010 | 17x |

Using only these 10 features, a logistic regression achieves **F1 = 0.847** (5-fold CV) for adversarial detection --- comparable to using the full 512-feature Mahalanobis distance.

### Causal Interventions --- Do These Features *Cause* Behavior Changes?

To move beyond correlational evidence, we performed activation patching through the SAE. For each of the 10 features, we ran inference on 20 successful jailbreak prompts, 20 refused prompts, and 20 benign prompts under three conditions: **(1)** normal, **(2)** feature ablated (clamped to zero), **(3)** feature amplified (3x).

| Condition | JB &rarr; Refused | Refused &rarr; Complied | Benign &rarr; Refused |
|---|---|---|---|
| Ablation (feature = 0) | **8.5%** | **17.0%** | 1.5% |
| Amplification (feature &times; 3) | **7.5%** | **19.5%** | 1.5% |

Key findings:

1. **Ablating individual features flips 8.5% of successful jailbreaks to refusals.** This is a modest but real causal effect --- individual features contribute incrementally, not as on/off switches.
2. **Refused prompts are 2x more sensitive** (17% flip rate). These prompts are "on the edge" --- the model's decision is fragile, and small activation changes tip the balance.
3. **Benign prompts are nearly immune** (1.5% flip rate). The features are specific to the adversarial decision boundary.
4. **The strongest single feature** (30000, "Safe instruction patterns") flips 15% of jailbreaks when ablated --- the most causally important individual feature.

This confirms that the discriminative SAE features are not merely correlated with adversarial behavior --- they are *mechanistically involved* in the model's decision to comply or refuse.

### Data Scaling --- How Much Baseline Data Is Needed?

We extracted SAE features for all 5,000 benign prompts and evaluated detection performance as a function of baseline size. For each N, we recomputed the Mahalanobis baseline from N randomly sampled benign prompts and measured F1 on a held-out test set (200 benign + 200 adversarial), averaged over 10 random subsamples.

| Baseline Size | Features Used | F1 (mean &plusmn; std) |
|---|---|---|
| 10 | 32 | 0.695 &plusmn; 0.048 |
| 50 | 32 | 0.653 &plusmn; 0.029 |
| 100 | 50 | 0.690 &plusmn; 0.027 |
| 200 | 100 | 0.706 &plusmn; 0.017 |
| 500 | 250 | 0.734 &plusmn; 0.014 |
| 1,000 | 500 | **0.764 &plusmn; 0.008** |
| 2,000 | 512 | 0.767 &plusmn; 0.005 |
| 4,800 | 512 | 0.758 &plusmn; 0.000 |

Performance scales log-linearly from N=10 to N=1,000, then **plateaus at F1 &approx; 0.76**. Beyond 1,000 benign prompts, additional data does not improve the Mahalanobis baseline. The bottleneck shifts from baseline quality to the fundamental overlap between benign and adversarial activation distributions. Variance also decreases steadily: at N=10 the spread is &plusmn;0.048; at N=1,000 it is &plusmn;0.008.
...
The causal intervention results confirm this mechanistically: the features that distinguish adversarial from benign prompts are not passive indicators --- perturbing them changes the model's behavior. The asymmetric sensitivity (refused > jailbroken > benign) reflects the model's decision landscape: refused prompts sit near a decision boundary that small perturbations can cross, while jailbroken prompts have found a more stable basin of compliance.
...
- [**Red-Team Campaign**](docs/animations/sotif_redteam.html) --- Round-by-round adversary evolution, SAE distance reveal, and trust region predictions.
- [**Envelope Evaluation**](docs/animations/sotif_evaluation.html) --- Per-prompt classification with live F1, SAE feature sliders, and data scaling study.
